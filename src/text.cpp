...
namespace sq {
std::vector<sf::Sprite> Writing::writeCharacters(const sf::Texture& font)
{
    int originalX = posX;
...
void Writing::set_string(const std::string& input)
{
    text = input;
    for (auto& it : text) {
        it = std::tolower(it);
...
std::string Writing::get_string() const
{
    return text;
}

//Maybe add stuff to move or squish when on the screen's edge
void Writing::set_pos(const int x, const int y)
{
    posX = x;
    posY = y;
}

void Writing::show(sf::RenderWindow& window, const sf::Texture& font)
{
...
Writing::Writing(const std::string&& input, int x, int y)
{
    set_string(input);
    posX = x;
    posY = y;
