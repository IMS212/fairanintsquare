...
namespace sq {
Game::Game(bool config)
    : window(sf::VideoMode(1366, 768), "Fair ain't square", sf::Style::Default)
    , textureManager()

{
    window.setFramerateLimit(60);
    textureManager.addTexture("font", "font.png");
}
...
void Game::start()
{
    ...
    while (window.isOpen()) {
        ...
        myText.show(window, textureManager.getTexture("font"));
        window.draw(square);
        window.display();
    }
}
